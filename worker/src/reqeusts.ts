...
export class AnimePahe {
	...
	private static Headers(streamUrl: string | false, userAgent: string) {
		return {
			...
			'referer': streamUrl ? `https://animepahe.org/anime/${ streamUrl }` : 'https://animepahe.org',
			'user-agent': userAgent,
		}
	}

	private async Series() {
		const res = /<h1[^>]*><span[^>]*>(?<title>[^<]+)<\/span>/.exec(await fetch(`https://animepahe.org/anime/${this.streamUrl}`, {
	...
	private async Extract(page: string | false) {
		return await fetch(`https://animepahe.org/api?m=release&id=${this.streamUrl}&sort=episode_asc&page=${page ? page : 1}`, {
			headers: AnimePahe.Headers(this.streamUrl, this.userAgent)
	...
	public async Links(session: string) {
		return await fetch(`https://animepahe.org/play/${this.streamUrl}/${session}`, {
			headers: AnimePahe.Headers(this.streamUrl, this.userAgent)
		}).then(async (res) => {
	...
	public static async search(query: string, userAgent: string) {
		const res = await fetch(`https://animepahe.org/api?m=search&q=${ query }`, {
